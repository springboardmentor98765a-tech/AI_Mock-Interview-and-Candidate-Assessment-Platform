...
public class InterviewEvaluationGeminiClient {
    ...
    public InterviewEvaluationResponse evaluateInterview(InterviewEvaluationRequest request) {
        ...
        try {
            ...
            InterviewEvaluationResponse parsed = parseEvaluation(textBuilder.toString(), request);
    ...
    private InterviewEvaluationResponse parseEvaluation(String rawResponse, InterviewEvaluationRequest request) {
        try {
    ...
    private InterviewEvaluationResponse buildFallbackEvaluation(InterviewEvaluationRequest request) {
        ...
        for (int i = 0; i < pairCount; i++) {
            String question = i < questions.size() ? safeText(questions.get(i)) : "";
            String answer = i < answers.size() ? safeText(answers.get(i)) : "";
            if (question.isBlank()) {
                continue;
            }
            if (answer.isBlank() || "No answer provided.".equalsIgnoreCase(answer)) {
                continue;
            }
            answeredQuestions++;
            effectiveQuestions.add(question);
            effectiveAnswers.add(answer);
        }

        // Never award a positive performance score when the candidate answered zero questions.
        ...
        int rawOverall = weightedOverall(
                response.getCommunicationScore(),
                response.getConfidenceScore(),
                response.getTechnicalScore(),
                response.getProfessionalismScore()
        );
        double completionRatio = totalQuestions > 0 ? ((double) answeredQuestions / (double) totalQuestions) : 1.0;
        response.setOverallScore(clamp((int) Math.round(rawOverall * completionRatio)));
    ...
    private InterviewEvaluationResponse buildNotAttemptedEvaluation(int totalQuestions) {
        ...
        response.setImprovementSuggestions(List.of("Complete and submit answers for the interview questions to receive a meaningful evaluation."));
        response.setPracticeRecommendations(List.of("Start another mock interview and answer at least one question."));
        response.setLearningResources(List.of("SmartHire interview preparation guide"));
        response.setFeedback(List.of("Interview submitted without any answered questions; performance cannot be meaningfully evaluated."));
        return response;
    }
