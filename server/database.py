...
def init_db():
    ...
    # Ensure the built-in local admin can always sign in on a fresh extraction.
    admin_email = next(iter(ADMIN_EMAILS), "admin@smarthire.local")
    admin_password = os.getenv("ADMIN_DEMO_PASSWORD", "Admin@12345")
    existing_admin = conn.execute("SELECT id FROM users WHERE lower(email)=lower(?)", (admin_email,)).fetchone()
    if not existing_admin:
        conn.execute(
            "INSERT INTO users (name, email, password, role, provider) VALUES (?, ?, ?, 'admin', 'LOCAL')",
            ("SmartHire Administrator", admin_email, _password_context.hash(admin_password)),
        )
