...
def row_to_user(row) -> dict:
    ...
    role = row["role"]
    # This local/demo build permits admin accounts to sign in and use the admin dashboard.
    # Production deployments should restrict admin creation to an approved allowlist.
    if role == "admin":
        role = "admin"
    return {
        "id": row["id"],
...
def register(req: RegisterRequest):
    if req.role not in ("candidate", "recruiter", "admin"):
        raise HTTPException(400, "Please select a valid account role.")
    ...
    try:
        ...
        if not user:
            ...
            if user:
                ...
                google_role = 'admin' if email.strip().lower() in ADMIN_EMAILS else 'candidate'
                cur = conn.execute(
                    "INSERT INTO users (name, email, role, provider, google_id, avatar) VALUES (?, ?, ?, 'GOOGLE', ?, ?)",
                    (name, email, google_role, google_id, avatar),
                )
                user = conn.execute("SELECT * FROM users WHERE id = ?", (cur.lastrowid,)).fetchone()
            conn.commit()
