...
def end_interview(interview_id: int, req: Optional[InterviewUpdateRequest] = None, user: dict = Depends(get_current_user)):
    payload = req or InterviewUpdateRequest()
    payload.status = "completed"
    return update_interview(interview_id, payload, user)
...
def _persist_vision_and_attention(interview_id: int, conn, release_attention: bool = True) -> dict:
    """Merge live vision, attention and behavior summaries into the interview row."""
    summary = vision_monitor.collect_and_clear_session(interview_id, full_timeline=True)
    merged = dict(summary or {})
    attention_summary = (
        attention_monitor.clear_session(interview_id)
        if release_attention
        else attention_monitor.collect_summary(interview_id)
    )
...
def analyze_vision_frame(interview_id: int, req: VisionFrameRequest, user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        attention = attention_monitor.record_frame(
            interview_id, eye_state, bool(result.get("face_present"))
        )

        if attention.get("should_terminate") and row["status"] == "in_progress":
            _persist_vision_and_attention(interview_id, conn, release_attention=False)
            conn.execute(
                "UPDATE interview_session SET status='completed', completed_at=CURRENT_TIMESTAMP, integrity_flag='potential_cheater' WHERE id=?",
                (interview_id,),
            )
            conn.commit()
            scoring_engine.generate_final_report(interview_id, conn)
        ...
        conn.close()
        raise HTTPException(500, "Vision analysis failed: " + str(exc))
...
def get_vision_summary(interview_id: int, user: dict = Depends(get_current_user)):
    conn = get_db()
    row = conn.execute(
        "SELECT id, vision_metrics_json, integrity_flag FROM interview_session WHERE id = ? AND (user_id = ? OR candidate_id = ?)",
        (interview_id, user["id"], user["id"]),
    ).fetchone()
    if not row:
        conn.close()
        raise HTTPException(404, "Interview not found.")
    stored = None
    if row["vision_metrics_json"]:
        try:
            stored = json.loads(row["vision_metrics_json"])
        except Exception:
            stored = None
    live = vision_monitor.get_summary(interview_id)
    attention = attention_monitor.get_snapshot(interview_id)
    conn.close()
    return {"live": live, "persisted": stored, "attention": attention, "integrity_flag": row["integrity_flag"]}
