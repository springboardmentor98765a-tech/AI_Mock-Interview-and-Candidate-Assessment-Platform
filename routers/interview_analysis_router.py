...
def start_analysis_session(req: StartAnalysisRequest):
    ...
    # Also initialize device detection session with metadata if matching interview in db
    interview = db.interviews.get(req.session_id)
    cand_id = interview.get("user_id") if interview else None
    device_detection_manager.get_or_create_session(
        session_id=req.session_id,
        candidate_id=cand_id,
        interview_id=req.session_id
    )
...
def analyze_video_frame(req: FrameAnalysisRequest):
    ...
    6. Real-Time YOLOv5n Electronic Device Detection & Anti-Cheating Telemetry
    7. Cumulative Session Metrics (Eye Contact %, Attention %, Engagement %, Confidence Indicators)
    """
    tracker = behavior_manager.get_or_create_session(req.session_id)

    # Execute computer vision facial pipeline
    ...
    # Execute real electronic device detection pipeline
    interview = db.interviews.get(req.session_id)
    cand_id = interview.get("user_id") if interview else None
    device_result = device_detection_manager.analyze_frame_for_session(
        session_id=req.session_id,
        image_data=req.frame_data,
        candidate_id=cand_id,
        interview_id=req.session_id
    )
...
def stop_analysis_session(req: StopAnalysisRequest):
    ...
    # Also stop device detection session
    dev_tracker = device_detection_manager.get_session(req.session_id)
    dev_summary = None
    if dev_tracker:
        dev_tracker.stop_session()
    ...
    return {
        ...
        "device_detection_summary": dev_summary
    }
...
def get_session_status(session_id: str):
    ...
    dev_tracker = device_detection_manager.get_session(session_id)
    dev_summary = dev_tracker.generate_summary_dict() if dev_tracker else None
...
def get_session_report(session_id: str):
    ...
    integrating video telemetry, electronic device monitoring, and speech metrics.
    """
    tracker = behavior_manager.get_session(session_id)
    if not tracker:
        raise HTTPException(status_code=404, detail="Analysis session not found.")
    ...
    # Attach device detection summary
    dev_tracker = device_detection_manager.get_session(session_id)
    if dev_tracker:
        report["device_detection"] = dev_tracker.generate_summary_dict()
    else:
