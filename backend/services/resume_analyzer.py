...
AUDIT_BLUEPRINT = [
    ...
    {"key": "bullet_style", "title": "Bullet length", "max": 10, "bucket": "Brevity"},
    {"key": "action_verbs", "title": "Action verbs", "max": 10, "bucket": "Impact"},
    {"key": "skills_keywords", "title": "Skills match", "max": 10, "bucket": "Skills"},
    {"key": "ats_parse", "title": "ATS readability", "max": 10, "bucket": "ATS"},
]


def _groq_configs():
    primary_key = (GROQ_API_KEY or "").strip()
    primary_model = (GROQ_RESUME_MODEL or "qwen/qwen3.6-27b").strip()
    secondary_key = (GROQ_API_KEY_2 or "").strip()
    secondary_model = (GROQ_RESUME_MODEL_2 or "openai/gpt-oss-120b").strip()

    configs = []
    if primary_key:
        configs.append((primary_key, primary_model))
    if secondary_key:
        configs.append((secondary_key, secondary_model))
    try:
        masked = [f"{m} (key ...{k[-4:]})" for k, m in configs]
