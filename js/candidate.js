...
function candidateOverview() {
  ...
  var ratingColor = activeRating === 'Excellent' ? EMERALD : activeRating === 'Good' ? CYAN : activeRating === 'Average' ? AMBER : ROSE;
  var history = data.history || [];
  var modalHtml = state.activeReportModal ? renderReportModal(state.activeReportModal) : '';

  var rubricTiers = [
    { range: '90–100%', label: 'Excellent', text: 'text-emerald-300', bg: 'bg-emerald-500/10', border: 'border-emerald-500/20', activeBg: 'background:rgba(31, 225, 161, 0.22);border:1.5px solid #12b67f' },
    { range: '75–89%', label: 'Good', text: 'text-indigo-300', bg: 'bg-indigo-500/10', border: 'border-indigo-500/20', activeBg: 'background:rgba(78, 80, 201, 0.22);border:1.5px solid #ffb199' },
    { range: '60–74%', label: 'Average', text: 'text-amber-300', bg: 'bg-amber-500/10', border: 'border-amber-500/20', activeBg: 'background:rgba(245,158,11,0.22);border:1.5px solid #f5b942' },
    { range: '40–59%', label: 'Needs-Improvement', text: 'text-rose-300', bg: 'bg-rose-500/10', border: 'border-rose-500/20', activeBg: 'background:rgba(244,63,94,0.22);border:1.5px solid #ff5d73' },
    { range: 'Below 40%', label: 'Poor', text: 'text-red-400', bg: 'bg-red-500/10', border: 'border-red-500/20', activeBg: 'background:rgba(225,29,72,0.25);border:1.5px solid #ff5d73' },
  ];

  return `<div class="space-y-6">
    <div><h1 class="text-2xl font-bold text-white" style="font-family:'Outfit',sans-serif">Good day, ${userName} 🤗</h1><p class="text-white/40 text-sm mt-1">Welcome to your SmartHire AI candidate dashboard.</p></div>
...
function renderResumeUploadArea() {
  ...
  if (status === 'uploading' || status === 'analyzing') {
    return `<div class="mic-resume-zone mic-resume-zone-active">
      <span class="mic-upload-icon animate-pulse" style="color:#ffb199">${icon('cpu', 16)}</span>
      <p class="mic-upload-main" style="color:#ffd0c2">${status === 'uploading' ? 'Uploading resume...' : 'Analyzing resume...'}</p>
    ...
    return `<div class="mic-resume-card">
      <div style="display:flex;align-items:center;gap:0.625rem;min-width:0">
        ...
        <div style="min-width:0">
          ...
          <p class="mic-file-meta">${res.size_mb} MB &bull; <span style="color:#84cc16;font-weight:600">✓ Resume ready</span></p>
        </div>
      </div>
  ...
  return `<div id="mic-resume-dropzone" class="mic-resume-zone ${isErr ? 'mic-resume-zone-error' : ''}">
    <input type="file" id="mic-resume-file" accept=".pdf,.docx" style="display:none" />
    <span class="mic-upload-icon" style="${isErr ? 'color:#ff5d73' : ''}">${icon(isErr ? 'alertTriangle' : 'uploadCloud', 16)}</span>
  ...
  return `<div class="mic-card">
    ...
    return `<button class="${chipClass(isActive, 'emerald')} config-interviewer-btn" data-interviewer="${ist}">${ist}</button>`;
      ...
      <div class="mic-summary">
        ...
        <span class="mic-sep">&bull;</span>
        <span><strong>Resume:</strong> <span style="${state.configResume ? 'color:#b7f7a1;font-weight:600' : ''}">${state.configResume ? 'Uploaded (' + state.configResume.filename + ')' : 'Not uploaded'}</span></span>
      </div>
  ...
  return `<div class="sh-room-container">
    ...
    <div class="sh-room-grid">
      ...
      <div class="sh-user-stage-card">
        ...
        <div class="mt-3 rounded-xl border border-cyan-500/20 bg-cyan-500/5 p-3">
          <div class="flex items-center justify-between gap-2 mb-2">
            <span class="text-[10px] font-bold uppercase tracking-wider text-cyan-300">Live Vision Monitor</span>
            <span id="vision-live-status" class="text-[10px] text-white/60">Initializing...</span>
          </div>
          <p id="vision-live-warning" class="text-[10px] text-amber-300 mb-2">Vision monitoring active</p>
          <div class="grid grid-cols-2 gap-2 text-[10px]">
            <div class="rounded-lg bg-white/5 p-2"><span class="text-white/40">Eye Contact</span><br><strong id="vision-live-eye" class="text-cyan-300">Analyzing</strong></div>
            <div class="rounded-lg bg-white/5 p-2"><span class="text-white/40">Emotion</span><br><strong id="vision-live-emotion" class="text-emerald-300">Analyzing</strong></div>
            <div class="rounded-lg bg-white/5 p-2"><span class="text-white/40">Attention</span><br><strong id="vision-live-attention" class="text-indigo-300">Monitoring</strong></div>
            <div class="rounded-lg bg-white/5 p-2"><span class="text-white/40">Confidence</span><br><strong id="vision-live-confidence" class="text-amber-300">Analyzing</strong></div>
          </div>
        </div>
      </div>
...
async function onRecordingStop() {
  // Manual submit may already be posting the live transcript. Do not submit twice.
  ...
  try {
    ...
    // Give browser speech recognition a moment to deliver its final result.
    ...
    for (var attempt = 0; attempt < 3; attempt++) {
      try {
        ...
        transcriptionError = e && e.message ? e.message : 'Cloud transcription unavailable';
        if (attempt < 2) { await new Promise(function (r) { setTimeout(r, 1200); }); }
      }
    }

    // Never discard a valid browser transcript just because cloud transcription failed.
    if (!transcript && state.currentTranscript) {
      transcript = state.currentTranscript.trim();
      console.warn('Using browser speech-recognition fallback:', transcriptionError);
    }
...
function stopVisionMonitoring() {
  ...
  state.visionCanvas = null;
}

function updateVisionLiveUI(result) {
  state.visionLive = result || null;
  var status = document.getElementById('vision-live-status');
  var warning = document.getElementById('vision-live-warning');
  var eye = document.getElementById('vision-live-eye');
  var emo = document.getElementById('vision-live-emotion');
  var att = document.getElementById('vision-live-attention');
  var conf = document.getElementById('vision-live-confidence');
  if (!result) return;
  if (status) {
    if (result.status === 'no_face') status.textContent = 'No face detected';
    else if (result.status === 'multiple_faces') status.textContent = 'Multiple faces detected';
    else if (result.status === 'face_detected') status.textContent = 'Face detected';
    else status.textContent = result.status || 'Vision monitoring';
  }
  if (warning) {
    if (result.status === 'no_face') warning.textContent = '⚠ Please stay visible in the camera.';
    else if (result.status === 'multiple_faces') warning.textContent = '⚠ Only the candidate should be visible.';
    else if (result.warnings && result.warnings.length) warning.textContent = '⚠ ' + result.warnings[0];
    else if (result.attention && result.attention.live_warning) warning.textContent = '⚠ ' + result.attention.live_warning;
    else warning.textContent = 'Vision monitoring active';
  }
  var eyeState = result.eye_contact && result.eye_contact.state;
  if (eye) eye.textContent = eyeState || 'Analyzing';
  if (emo) emo.textContent = result.emotion && result.emotion.dominant ? result.emotion.dominant : 'Analyzing';
  if (att) att.textContent = result.attention && result.attention.state ? result.attention.state : 'Monitoring';
  if (conf) {
    var ci = result.summary && result.summary.confidence_indicator;
    conf.textContent = ci && ci.score != null ? Math.round(ci.score) + '%' : 'Analyzing';
  }
  // Replace the old simulated eye/emotion telemetry with actual vision results.
  var eyeVal = document.getElementById('telemetry-eye-val');
  var eyePct = result.summary && result.summary.eye && result.summary.eye.contact_pct;
  if (eyeVal && typeof eyePct === 'number') eyeVal.textContent = Math.round(eyePct) + '%';
  var emoVal = document.getElementById('telemetry-emo-val');
  if (emoVal && result.emotion && result.emotion.dominant) emoVal.textContent = result.emotion.dominant;
}

function startVisionMonitoring() {
  stopVisionMonitoring();
  var interview = state.currentInterview && state.currentInterview.interview;
  if (!interview) return;
  var canvas = document.createElement('canvas');
  state.visionCanvas = canvas;
  var busy = false;
  async function captureFrame() {
    if (busy) return;
    var video = document.getElementById('candidate-camera');
    if (!video || !state.interviewStream || !video.videoWidth || !video.videoHeight) return;
    var tracks = state.interviewStream.getVideoTracks ? state.interviewStream.getVideoTracks() : [];
    if (!tracks.length || tracks[0].readyState !== 'live' || tracks[0].enabled === false) return;
    busy = true;
    try {
      ...
      var result = await api.analyzeVisionFrame(interview.id, imageData);
      updateVisionLiveUI(result);
      if (result && result.attention && result.attention.terminated) {
        stopVisionMonitoring();
        state.currentInterview.interview.status = 'completed';
        render();
      }
  ...
  captureFrame();
  state.visionTimer = setInterval(captureFrame, 1500);
}
...
function bindCandidateInterviewEvents() {
  ...
  async function submitCurrentAnswerDirect() {
    if (state.answerSubmitting) return false;
    var interview = state.currentInterview && state.currentInterview.interview;
    var questions = state.currentInterview && state.currentInterview.questions;
    var transcript = (state.currentTranscript || '').trim();
    var q = questions && questions[state.currentQuestionIndex];
    if (!interview || !q || !transcript || transcript.length < 3) return false;
    state.answerSubmitting = true;
    setSessionStatus('Evaluating answer with AI...', 'text-indigo-300');
    try {
      if (interview.status === 'paused') {
        var resumed = await api.resumeInterview(interview.id);
        if (resumed && resumed.interview) state.currentInterview.interview = resumed.interview;
      }
      var durationSec = state.answerStartTime ? Math.max(2, Math.round((Date.now() - state.answerStartTime) / 1000)) : 15;
      var words = transcript.split(/\s+/).filter(Boolean).length;
      var wpm = Math.min(240, Math.round(words / (durationSec / 60)));
      var result = await api.submitInterviewAnswer(interview.id, q.id, transcript, durationSec, wpm);
      state.currentInterview.interview = result.interview || state.currentInterview.interview;
      state.currentInterview.questions[state.currentQuestionIndex] = result.question || q;
      state.currentTranscript = transcript;
      state.answerSubmitting = false;
      setSessionStatus('Answer submitted and evaluated.', 'text-emerald-300');
      render();
      setTimeout(nextQuestion, 1200);
      return true;
    } catch (err) {
      state.answerSubmitting = false;
      setSessionStatus('Answer submission failed: ' + (err.message || 'Unknown error'), 'text-rose-400');
      var box = document.getElementById('transcript-box');
      if (box) box.innerHTML += '<p class="text-rose-400 text-xs mt-2">Answer was not submitted: ' + (err.message || 'Unknown error') + '</p>';
      return false;
    }
  }
  ...
  if (btnManualSubmit) {
    btnManualSubmit.addEventListener('click', async function () {
      if (state.isAiSpeaking || state.answerSubmitting) return;
      btnManualSubmit.disabled = true;
      btnManualSubmit.classList.add('opacity-60');
      stopAutoRecording();
      // Submit the already visible live transcript immediately. This does not
      // depend on cloud STT or on MediaRecorder's asynchronous onstop event.
      var submitted = await submitCurrentAnswerDirect();
      if (!submitted) {
        btnManualSubmit.disabled = false;
        btnManualSubmit.classList.remove('opacity-60');
        if (!state.currentTranscript) setSessionStatus('Speak an answer before submitting.', 'text-amber-300');
      }
    });
  }
...
function renderRubricBadge(rating, score) {
  ...
  var style = 'background:rgba(16,185,129,0.15);color:#84cc16;border:1px solid rgba(16,185,129,0.3)';
  if (r === 'Good') style = 'background:rgba(99,102,241,0.15);color:#ffb199;border:1px solid rgba(99,102,241,0.3)';
  else if (r === 'Average') style = 'background:rgba(245,158,11,0.15);color:#f5b942;border:1px solid rgba(245,158,11,0.3)';
  else if (r === 'Needs Improvement') style = 'background:rgba(244,63,94,0.15);color:#ff5d73;border:1px solid rgba(244,63,94,0.3)';
...
function renderReportModal(report) {
  ...
  var sections = [
    ...
    { id: 'report-vision', label: 'Vision & Behavior' },
  ...
  return `<div id="report-modal-overlay" class="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto report-overlay" style="background:rgba(0,0,0,0.8);backdrop-filter:blur(6px)">
    <div class="w-full max-w-5xl my-6 lg:my-10 rounded-2xl border border-white/10 overflow-hidden report-modal-card" style="background:#0d0f1e">
      ...
      return `<div class="p-4 rounded-2xl border border-white/8 flex flex-col justify-between space-y-3" style="background:#0c0e1c">
        ...
        <div class="report-section" id="report-speech-grammar">
          <div class="flex items-center justify-between gap-3 mb-4">
            <div class="flex items-center gap-2.5">
              <span class="w-8 h-8 rounded-xl flex items-center justify-center" style="background:rgba(99,102,241,0.15);color:#ffd0c2;border:1px solid rgba(99,102,241,0.3)">${icon('messageSquare', 16)}</span>
              <div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div class="p-5 rounded-2xl border border-white/8 flex flex-col justify-between" style="background:#0c0e1c">
              <div>
                <div class="flex items-center justify-between gap-2 mb-3">
                  <div class="flex items-center gap-2">
                    <span class="w-7 h-7 rounded-lg flex items-center justify-center shrink-0" style="background:rgba(99,102,241,0.15);color:#ffd0c2;border:1px solid rgba(99,102,241,0.25)">${icon('checkCircle', 13)}</span>
                    <div>
            ...
            <div class="p-5 rounded-2xl border border-white/8 flex flex-col justify-between" style="background:#0c0e1c">
              <div>
                <div class="flex items-center justify-between gap-2 mb-3">
                  <div class="flex items-center gap-2">
                    <span class="w-7 h-7 rounded-lg flex items-center justify-center shrink-0" style="background:rgba(245,158,11,0.15);color:#f5b942;border:1px solid rgba(245,158,11,0.25)">${icon('activity', 13)}</span>
                    <div>
            ...
            <div class="p-5 rounded-2xl border border-white/8 flex flex-col justify-between" style="background:#0c0e1c">
              <div>
                <div class="flex items-center justify-between gap-2 mb-3">
                  <div class="flex items-center gap-2">
                    <span class="w-7 h-7 rounded-lg flex items-center justify-center shrink-0" style="background:rgba(6,182,212,0.15);color:#9af5e9;border:1px solid rgba(6,182,212,0.25)">${icon('headphones', 13)}</span>
                    <div>
          ...
          <div class="rounded-2xl border border-indigo-500/30 p-5 mt-4 relative overflow-hidden" style="background:linear-gradient(180deg,rgba(99,102,241,0.08),rgba(12,14,28,0.95))">
            <div class="flex flex-wrap items-center justify-between gap-3 mb-4 pb-3 border-b border-white/8">
              <div class="flex items-center gap-2.5">
                <span class="w-8 h-8 rounded-xl flex items-center justify-center shrink-0" style="background:rgba(99,102,241,0.2);color:#ffd0c2;border:1px solid rgba(99,102,241,0.4)">${icon('messageSquare', 16)}</span>
                <div>
      ...
      return `<div class="rounded-2xl border border-white/8 p-4" style="background:#0c0e1c">
        ...
        <div class="report-section grid grid-cols-1 md:grid-cols-2 gap-4" id="report-gaps">
          <div class="rounded-2xl border border-emerald-500/20 p-5" style="background:linear-gradient(180deg,rgba(16,185,129,0.06),transparent);background-color:#0c0e1c">
            <div class="flex items-center gap-2 mb-3">
              <span class="w-7 h-7 rounded-lg flex items-center justify-center" style="background:rgba(16,185,129,0.15);color:#84cc16">${icon('checkCircle2', 15)}</span>
          ...
          <div class="rounded-2xl border border-amber-500/20 p-5" style="background:linear-gradient(180deg,rgba(245,158,11,0.05),transparent);background-color:#0c0e1c">
            <div class="flex items-center gap-2 mb-3">
              <span class="w-7 h-7 rounded-lg flex items-center justify-center" style="background:rgba(245,158,11,0.15);color:#f5b942">${icon('alertTriangle', 15)}</span>
        ...
        <div class="report-section" id="report-plan">
          <div class="flex items-center gap-2 mb-3">
            <span class="w-7 h-7 rounded-lg flex items-center justify-center" style="background:rgba(99,102,241,0.15);color:#ffd0c2">${icon('lightbulb', 15)}</span>
      ...
      return `<div class="flex items-start gap-2.5">
                    <span class="w-5 h-5 rounded-md flex items-center justify-center text-[10px] font-bold shrink-0" style="background:rgba(99,102,241,0.15);color:#ffd0c2">${String(i + 1).padStart(2, '0')}</span>
      ...
      return `<div class="flex items-start gap-2.5">
                    <span class="w-5 h-5 rounded-md flex items-center justify-center text-[10px] font-bold shrink-0" style="background:rgba(6,182,212,0.15);color:#9af5e9">${String(i + 5).padStart(2, '0')}</span>
                    <p class="text-xs text-white/80 leading-relaxed">${imp}</p>
                  </div>`;
      ...
      return `<div class="flex items-start gap-2.5">
                    <span class="w-5 h-5 rounded-md flex items-center justify-center text-[10px] font-bold shrink-0" style="background:rgba(16,185,129,0.15);color:#84cc16">${String(i + 1).padStart(2, '0')}</span>
        ...
        <!-- 8. Vision & Behavior -->
        ${report.vision_metrics ? (function () {
          var vm = report.vision_metrics || {};
          var eye = vm.eye || {};
          var att = vm.attention || {};
          var ci = vm.confidence_indicator || {};
          var eng = vm.engagement || {};
          var emo = vm.emotion || {};
          var behavior = vm.behavior || {};
          var dist = emo.dominant_distribution || {};
          var face = vm.face_detected_pct != null ? vm.face_detected_pct : 0;
          var multi = vm.multiple_faces_pct != null ? vm.multiple_faces_pct : 0;
          var warningCount = att.warnings || 0;
          var contact = eye.contact_pct != null ? eye.contact_pct : 0;
          var attentionPct = eng.components && eng.components.attention != null ? eng.components.attention : (ci.components && ci.components.attention != null ? ci.components.attention : 0);
          var engagement = eng.score != null ? eng.score : 0;
          var conf = ci.score != null ? ci.score : 0;
          return `<div class="report-section" id="report-vision">
            <div class="flex items-center gap-2 mb-3">
              <span class="w-7 h-7 rounded-lg flex items-center justify-center" style="background:rgba(6,182,212,0.15);color:#9af5e9">${icon('eye', 15)}</span>
              <div><h3 class="text-base font-semibold text-white" style="font-family:'Outfit',sans-serif">Vision &amp; Behavior</h3><p class="text-[10px] text-white/40">Measured from the interview webcam and attention monitor</p></div>
            </div>
            <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
              ${[['Face Visibility',face],['Eye Contact',contact],['Attention',attentionPct],['Confidence Indicator',conf]].map(function(x){return `<div class="rounded-xl border border-white/8 p-3" style="background:#0c0e1c"><p class="text-[10px] text-white/40 uppercase">${x[0]}</p><p class="text-xl font-bold text-white mt-1">${Number(x[1]||0).toFixed(0)}%</p></div>`}).join('')}
            </div>
            <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mt-3">
              <div class="rounded-xl border border-white/8 p-3" style="background:#0c0e1c"><p class="text-[10px] text-white/40 uppercase">Engagement</p><p class="text-xl font-bold text-white mt-1">${Number(engagement).toFixed(0)}%</p></div>
              <div class="rounded-xl border border-white/8 p-3" style="background:#0c0e1c"><p class="text-[10px] text-white/40 uppercase">Multiple Faces</p><p class="text-xl font-bold text-white mt-1">${Number(multi).toFixed(0)}%</p></div>
              <div class="rounded-xl border border-white/8 p-3" style="background:#0c0e1c"><p class="text-[10px] text-white/40 uppercase">Warnings</p><p class="text-xl font-bold text-white mt-1">${warningCount}</p></div>
              <div class="rounded-xl border border-white/8 p-3" style="background:#0c0e1c"><p class="text-[10px] text-white/40 uppercase">Integrity</p><p class="text-sm font-bold text-white mt-2">${report.integrity_flag || 'Normal'}</p></div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
              <div class="rounded-xl border border-white/8 p-4" style="background:#0c0e1c"><p class="text-[10px] text-white/40 uppercase mb-2">Emotion Distribution</p>${Object.keys(dist).length ? Object.keys(dist).map(function(k){return `<div class="flex justify-between text-xs py-1"><span class="text-white/70 capitalize">${k}</span><strong class="text-white">${dist[k]}%</strong></div>`}).join('') : '<p class="text-xs text-white/40">No emotion samples recorded.</p>'}</div>
              <div class="rounded-xl border border-white/8 p-4" style="background:#0c0e1c"><p class="text-[10px] text-white/40 uppercase mb-2">Behavior Analysis</p>${behavior.summary_points && behavior.summary_points.length ? behavior.summary_points.map(function(x){return `<p class="text-xs text-white/70 py-1">• ${typeof x === 'object' ? (x.text || x.message || JSON.stringify(x)) : x}</p>`}).join('') : '<p class="text-xs text-white/40">No behavior summary available.</p>'}</div>
            </div>
          </div>`;
        })() : ''}
        ...
        ${resources.length ? `<div class="report-section" id="report-resources">
          <div class="flex items-center gap-2 mb-3">
            <span class="w-7 h-7 rounded-lg flex items-center justify-center" style="background:rgba(6,182,212,0.15);color:#9af5e9">${icon('bookOpen', 15)}</span>
      ...
      return `<div class="p-4 rounded-2xl border border-white/8 flex flex-col justify-between transition-colors hover:border-white/15" style="background:#0c0e1c">
                <div>
                  <div class="flex items-center justify-between mb-2">
                    <span class="px-2 py-0.5 rounded text-[10px] font-semibold uppercase tracking-wider" style="background:rgba(99,102,241,0.15);color:#ffd0c2;border:1px solid rgba(99,102,241,0.25)">${res.type || 'Resource'}</span>
                  </div>
        ...
        ${questions.length ? `<div class="report-section" id="report-questions">
          <div class="flex flex-wrap items-center justify-between gap-3 mb-3">
            <div class="flex items-center gap-2">
              <span class="w-7 h-7 rounded-lg flex items-center justify-center" style="background:rgba(99,102,241,0.15);color:#ffd0c2">${icon('messageSquare', 15)}</span>
...
function reportRatingStyle(r) {
  if (r === 'Excellent') return 'background:rgba(16,185,129,0.15);color:#84cc16;border:1px solid rgba(16,185,129,0.3)';
  if (r === 'Good') return 'background:rgba(99,102,241,0.15);color:#ffb199;border:1px solid rgba(99,102,241,0.3)';
  if (r === 'Average') return 'background:rgba(245,158,11,0.15);color:#f5b942;border:1px solid rgba(245,158,11,0.3)';
  if (r === 'Needs Improvement') return 'background:rgba(244,63,94,0.15);color:#ff5d73;border:1px solid rgba(244,63,94,0.3)';
  ...
  return `<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div class="rounded-xl border border-white/7 p-5" style="background:#0d0f1e">
        ...
        <div class="space-y-3">
          ${[
      { range: '90 - 100%', label: 'Excellent', color: '#84cc16' },
      { range: '75 - 89%', label: 'Good', color: '#ffb199' },
      { range: '60 - 74%', label: 'Average', color: '#f5b942' },
      { range: '40 - 59%', label: 'Needs Improvement', color: '#ff5d73' },
...
function candidateHistory() {
  ...
  return `<div class="space-y-6">
    ...
    return `<tr class="border-b border-white/4 history-row transition-colors">
          <td class="px-5 py-4">
            <div class="flex items-center gap-3">
              <div class="w-9 h-9 rounded-lg flex items-center justify-center text-xs font-bold shrink-0" style="background:rgba(99,102,241,0.12);color:#ffb199;border:1px solid rgba(99,102,241,0.25)">#${i.id}</div>
              <div class="min-w-0">
  ...
  return `<div class="mic-page">
      ...
      <div class="mic-body">
        ...
        <div class="mic-field">
          <div style="display:flex;align-items:center;justify-content:space-between">
            ...
            <span style="font-size:0.65rem;color:#ffb199;font-weight:600">${topics.length} selected</span>
          </div>
          <div class="mic-chips">
    ...
    return `<button class="mic-chip ${isSel ? 'active' : ''} btn-assessment-time" data-time="${m}">${m} min</button>`;
            ...
            <div class="mic-resume-card">
              <div style="display:flex;align-items:center;gap:0.625rem;min-width:0">
                ...
                <div style="min-width:0">
                  ...
                  <p class="mic-file-meta"><span style="color:#84cc16;font-weight:600">✓ Resume ready</span> &bull; ${state.assessmentResume.skills ? state.assessmentResume.skills.length + ' skills' : 'Context ready'}</p>
                </div>
              </div>
              <div style="display:flex;align-items:center;gap:0.375rem;flex-shrink:0">
        ...
        <div class="mic-summary">
          ...
          <span class="mic-sep">&bull;</span>
          <span><strong>Resume:</strong> <span style="${state.assessmentResume ? 'color:#b7f7a1;font-weight:600' : ''}">${state.assessmentResume ? 'Ready' : 'None'}</span></span>
        </div>
