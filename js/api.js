...
async function apiRequest(endpoint, options) {
  ...
  if (!response.ok) {
    var serverError = data && (data.detail || data.error || data.message);
    if (serverError && typeof serverError === 'object') {
      serverError = serverError.map ? serverError.map(function(item) { return item.msg || item.message || JSON.stringify(item); }).join('; ') : JSON.stringify(serverError);
    }
    throw new Error(String(serverError || 'Request failed.'));
  }
  return data;
}
...
var api = {
  ...
  endInterview: function(id, elapsedSeconds) {
    return apiRequest('/interviews/' + id + '/end', {
      method: 'POST',
      body: { elapsed_seconds: elapsedSeconds || 0 }
    });
  },
